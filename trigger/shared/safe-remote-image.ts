...
async function readLimitedResponse(response: Response, sourceUrl: string) {
  ...
  if (!response.body) {
    ...
    return `data:${contentType};base64,${fallback.toString("base64")}`;
  }
...
export async function fetchSafeRemoteImageDataUrl(rawUrl: string, redirectCount = 0): Promise<string> {
  ...
  if ([301, 302, 303, 307, 308].includes(response.status)) {
    ...
    return fetchSafeRemoteImageDataUrl(new URL(location, url).toString(), redirectCount + 1);
  }
  ...
  return readLimitedResponse(response, url.toString());
}
