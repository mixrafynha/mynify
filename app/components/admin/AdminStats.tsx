  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
      <div className="flex items-center gap-3">
        <div className="rounded-xl bg-white/5 p-2 text-white/70">{icon}</div>
...
export default function AdminStats({ users, products, revenue }: any) {
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
      <Card icon={<Users size={15} />} label="Users" value={users?.length ?? 0} />
      ...
      <Card icon={<Euro size={15} />} label="Revenue" value={`€${revenue ?? 0}`} />
    </div>
  );
}
