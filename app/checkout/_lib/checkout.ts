...
export type CartVariant = {
  ...
  productUidV4?: string | null;
  product_uid_v4?: string | null;
  gelato_product_uid?: string | null;
  gelatoProductUid?: string | null;
  gelatoProductUidV4?: string | null;
  gelato_product_uid_v4?: string | null;
...
function valueFromRecord(
  ...
  for (const key of keys) {
    const direct = cleanNullableString(record[key]);
...
function resolveProductUid(
  ...
  return (
    valueFromRecord(item, keys) ??
    valueFromRecord(variant, keys) ??
    valueFromRecord(item.product, keys) ??
    valueFromRecord(item.design_data, keys) ??
    valueFromRecord(item.designData, keys) ??
    valueFromRecord(item.production, keys)
  );
}
...
function normalizeGelatoFileType(type: string): string {
  const normalized = type.trim().toLowerCase();
  if (normalized === "front") return "default";
  if (
    normalized === "default" ||
    normalized === "back" ||
    normalized === "neck-inner" ||
    normalized === "neck-outer" ||
    normalized === "sleeve-left" ||
    normalized === "sleeve-right" ||
    normalized === "inside"
  )
    return normalized;
...
function pushPrintFile(
  files: GelatoCheckoutPrintFile[],
  type: string,
  url: unknown,
) {
  if (!isPublicFileUrl(url)) return;
  const normalizedType = normalizeGelatoFileType(type);
  if (
    files.some(
      (file) => file.type === normalizedType && file.url === url.trim(),
    )
  )
    return;
...
function collectPrintFilesFromUnknown(
  files: GelatoCheckoutPrintFile[],
  value: unknown,
) {
  if (!value) return;
  ...
  if (Array.isArray(value)) {
    value.forEach((entry) => {
      ...
      pushPrintFile(
        files,
        cleanNullableString(record.type) ??
          cleanNullableString(record.side) ??
          "default",
        record.url ?? record.fileUrl ?? record.printFileUrl ?? record.print_file_url,
      );
    });
    return;
  }
  ...
  if (typeof value === "object") {
    Object.entries(value as Record<string, unknown>).forEach(([side, entry]) => {
      if (typeof entry === "string") {
        pushPrintFile(files, side, entry);
        return;
      }

      if (!entry || typeof entry !== "object") return;
      const sideRecord = entry as PrintSideRecord;
      pushPrintFile(
        files,
        side,
        sideRecord.printFileUrl ??
          sideRecord.print_file_url ??
          sideRecord.designImageUrl ??
          sideRecord.design_image_url,
      );
    });
  }
}

function collectNestedPrintFiles(
  files: GelatoCheckoutPrintFile[],
  value: unknown,
) {
  ...
  collectPrintFilesFromUnknown(files, record.files);
  collectPrintFilesFromUnknown(files, record.printFiles);
  collectPrintFilesFromUnknown(files, record.print_files);

  if (record.sides && typeof record.sides === "object") {
    collectPrintFilesFromUnknown(files, record.sides);
  }

  if (record.production && typeof record.production === "object") {
    collectNestedPrintFiles(files, record.production);
  }
}
...
function resolveGelatoPrintFiles(item: CartItem): GelatoCheckoutPrintFile[] {
  ...
  collectPrintFilesFromUnknown(files, item.print_files);
  collectNestedPrintFiles(files, item.design_data);
  collectNestedPrintFiles(files, item.designData);
  collectNestedPrintFiles(files, item.production);
  collectNestedPrintFiles(files, item.product);
  pushPrintFile(
    files,
    "default",
    item.printFileUrl ??
      item.print_file_url ??
      item.frontPrintFileUrl ??
      item.front_print_file_url,
  );
  pushPrintFile(
    files,
    "back",
    item.backPrintFileUrl ?? item.back_print_file_url,
  );
  return files;
}

function createOrderReferenceId(): string {
  if (typeof crypto !== "undefined" && "randomUUID" in crypto)
    return `ryfio-checkout-${crypto.randomUUID()}`;
  return `ryfio-checkout-${Date.now()}-${Math.random().toString(36).slice(2)}`;
}

function createItemReferenceId(item: CartItem, index: number): string {
  const stableId =
    item.id ||
    item.variant_id ||
...
export function createCheckoutRequestPayload(
  ...
  return {
    ...
    items: items.map((item, index) => {
      ...
      return {
        itemReferenceId: createItemReferenceId(item, index),
        id: item.id,
        product_id: getCartProductId(item),
        user_product_id: item.user_product_id ?? item.userProductId ?? null,
        base_product_id: item.base_product_id ?? item.baseProductId ?? null,
        ...
        product_color_id:
          variantProductColorId(currentVariant) ??
...
export function resolveCheckoutCountry(value?: string | null) {
  ...
  if (aliased)
    return GELATO_COUNTRIES.find((item) => item.country === aliased) || null;

  return (
    GELATO_COUNTRIES.find(
...
export function readCheckoutSession(): CheckoutForm | null {
  ...
  try {
    ...
    return {
      ...
      phoneCountry:
        typeof data.phoneCountry === "string" ? data.phoneCountry : "+351",
...
export function variantId(variant: CartVariant) {
  return (
    variant.id ||
    variant.variant_id ||
    `${variantColor(variant) || ""}-${variantSize(variant) || ""}-${variant.sku || ""}`
  );
}
...
export function variantColor(variant: CartVariant) {
  return (
    ...
    variant.product_color?.color_name ||
    variant.product_color?.name ||
    variant.product_color?.title ||
    variant.product_colors?.color ||
    variant.product_colors?.color_name ||
    variant.product_colors?.name ||
    variant.product_colors?.title ||
    null
  );
}
...
export function variantHex(variant: CartVariant) {
  return (
    ...
    variant.product_color?.hex ||
    variant.product_colors?.color_hex ||
    variant.product_colors?.hex ||
    "#9ca3af"
  );
}
...
export function variantImage(variant: CartVariant | null | undefined) {
  return (
    ...
    variant?.product_color?.image_url ||
    variant?.product_colors?.image ||
    variant?.product_colors?.image_url ||
    null
  );
}

export function variantPrice(
  variant: CartVariant | null | undefined,
  fallback = 0,
) {
  if (!variant) return fallback;
  return (
    toNumber(variant.price) ??
    toNumber(variant.final_price) ??
    toNumber(variant.base_price) ??
    fallback
  );
}
...
export function variantProductColorId(variant: CartVariant | null | undefined) {
  return (
    ...
    variant?.product_color?.id ||
    variant?.product_colors?.id ||
    null
  );
}
...
export function normalizeVariantLabel(variant: CartVariant) {
  return (
    [variantColor(variant), variantSize(variant)].filter(Boolean).join(" / ") ||
    "Variant"
  );
}
