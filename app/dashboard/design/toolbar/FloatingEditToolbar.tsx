  ...
  return (
    <>
      <div
        ...
        className="absolute left-1/2 top-3 z-[9999] hidden w-[min(calc(100vw-360px),980px)] -translate-x-1/2 select-none rounded-2xl border border-white/10 bg-[#070817]/94 px-2.5 py-2 text-white shadow-[0_18px_70px_rgba(0,0,0,.42)] backdrop-blur-2xl md:block"
      >
    ...
    <div className="relative shrink-0">
      <button
        ...
        className={mobile
          ...
          : "flex h-8 w-[170px] items-center justify-between gap-2 rounded-xl border border-white/10 bg-[#0c0e1d] px-2 text-left text-xs font-bold text-white outline-none hover:border-violet-300/35"}
        title="Font family"
      >
      ...
      {open && (
        <div
          ...
          className={mobile
            ...
            : "absolute left-0 top-10 z-[10030] w-[280px] overflow-hidden rounded-2xl border border-white/10 bg-[#080916]/98 p-2 shadow-[0_22px_70px_rgba(0,0,0,.48)] backdrop-blur-2xl"}
        >
          <input
            value={query}
            ...
            {fonts.map((font) => {
              ...
              return (
                <button
                  ...
                  className={`flex h-9 w-full items-center justify-between gap-2 rounded-xl px-2 text-left text-xs font-bold transition ${active ? "bg-violet-500 text-white" : "text-white/78 hover:bg-white/[0.075]"}`}
                >
                  <span className="truncate">{font.family}</span>
                  <span className="shrink-0 text-[10px] capitalize text-white/40">{font.category}</span>
                </button>
              );
            })}
          </div>
...
function ColorChip({ value, onChange, title }: { value: string; onChange: (value: string) => void; title: string }) {
  return (
    <label className="relative flex h-8 w-8 shrink-0 cursor-pointer items-center justify-center rounded-xl border border-white/10 bg-white/[0.055]" title={title}>
