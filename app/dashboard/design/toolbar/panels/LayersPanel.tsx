...
export default function LayersPanel({ elements = [], selected, setSelectedId, setSelectedElement, updateElement, deleteElement }: Props) {
  ...
  if (!sorted.length) return <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-5 text-center text-sm font-bold text-slate-400">No layers yet.</div>;
  return <div className="space-y-2 pb-6">{sorted.map((el) => {
    ...
    return <div key={el.id} className={`flex min-h-12 items-center gap-2 rounded-2xl border px-3 ${active ? "border-violet-400 bg-violet-500/20" : "border-white/10 bg-white/[0.045]"}`}>
