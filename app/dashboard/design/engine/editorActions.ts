...
export function reorderElement(
  ...
  return elements.map((item) => {
    ...
    if (action === "sendBackward") {
      return { ...item, zIndex: Math.max(0, (item.zIndex || 0) - 1) };
    }
...
export function buildTextElement(
  ...
  return {
    ...
    text,
    content: text,
    width: Math.max(128, text.length * 22),
    height: 60,
    fontFamily,
    fontSize: 36,
    fontWeight: 800,
