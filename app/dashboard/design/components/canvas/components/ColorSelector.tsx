...
function ColorSelector({
  ...
  return (
    <div
      data-ryfio-canvas-ui="true"
      className="absolute left-3 top-3 z-[72] md:left-4 md:top-4"
                ...
                return (
                  <button
                    ...
                    className={`relative h-[22px] w-[22px] shrink-0 rounded-full border transition active:scale-95 md:h-8 md:w-8 ${
                      selected
                        ? "border-white ring-2 ring-violet-300/80"
                        : "border-white/20 hover:border-white/60"
                    }`}
                    style={{ backgroundColor: color.hex }}
                  >
                    {selected && (
                      <span className="absolute inset-[2px] rounded-full border border-black/25 md:inset-[4px]" />
                    )}
                  </button>
                );
              })}
