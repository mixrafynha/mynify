...
function MobileToolbar({ open, panel, setOpen, setPanel }: MobileToolbarProps) {
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-[90] md:hidden">
      ...
      <nav aria-label="Editor tools" onClick={(event) => event.stopPropagation()} className={`pointer-events-auto fixed bottom-[max(5px,env(safe-area-inset-bottom))] left-1/2 z-[91] h-[58px] w-[calc(100%_-_14px)] max-w-[520px] -translate-x-1/2 rounded-[20px] border border-violet-300/25 bg-[linear-gradient(180deg,rgba(28,16,52,.94)_0%,rgba(9,8,20,.92)_100%)] px-2 py-1 text-white shadow-[0_10px_28px_rgba(0,0,0,.38),0_0_26px_rgba(168,85,247,.18)] backdrop-blur-2xl transition-[transform,opacity] duration-300 ease-[cubic-bezier(.2,.9,.2,1)] ${menuOpen ? "translate-y-0 opacity-100" : "pointer-events-none translate-y-[calc(100%+40px)] opacity-0"}`}>
