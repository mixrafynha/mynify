        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-xs font-medium text-slate-400">
              {isText ? selected.text || "Text element" : `${Math.round(width)} × ${Math.round(height)}px`}
            </p>
          </div>
        </div>
      ...
      <Card title="Layer">
        <div className="grid grid-cols-2 gap-2">
          <ActionButton
            icon={<MoveUp size={16} />}
            label="Forward"
            onClick={() => updateElement({ layerAction: "forward" })}
          />

          <ActionButton
            icon={<MoveDown size={16} />}
            label="Backward"
            onClick={() => updateElement({ layerAction: "backward" })}
          />
