...
export default function UploadPanel({ onUpload }: UploadPanelProps) {
  const [status, setStatus] = useState(
    "PNG, JPG or WEBP • High resolution • Up to 25MB"
  );
  ...
  const handleFileChange = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isValidType) {
      setStatus("Invalid file format. Please use PNG, JPG or WEBP.");
      e.target.value = "";
      return;
    }
    ...
    if (file.size > PRINT_IMAGE_LIMITS.maxBytes) {
      setStatus(
        `Image is too large. Maximum ${bytesToMb(
          PRINT_IMAGE_LIMITS.maxBytes
        )}MB.`
      );
      e.target.value = "";
      return;
    ...
    try {
      ...
      if (!quality.ok) {
        setStatus(quality.error ?? "Invalid image.");
        e.target.value = "";
        return;
      }
  ...
  return (
    <div className="space-y-3 pb-4 text-slate-950">
      <label className="group relative flex min-h-[150px] w-full cursor-pointer flex-col items-center justify-center overflow-hidden rounded-2xl bg-slate-100 px-5 text-center shadow-none ring-1 ring-slate-200 transition active:scale-[0.985]">
        ...
        <div className="mt-3">
          <div className="text-base font-black leading-none tracking-[-0.03em]">
            Upload print-ready
          </div>

          <div className="mt-2 flex items-center justify-center gap-1.5 text-xs font-semibold text-slate-500">
            <Image size={14} />
            High DPI without aggressive compression
          </div>
        </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-2 text-[11px] font-semibold leading-5 text-slate-500">
              Recommended: shortest side at least 2400px for professional print
              quality on large products.
            </p>
          </div>
        </div>
      </div>
