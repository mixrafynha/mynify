...
function StickersPanel({ createElement }: { createElement?: (element: any) => void }) {
  ...
  return (
    <div className="space-y-4 pb-6">
      ...
      <div className="grid grid-cols-6 gap-2 sm:grid-cols-8 md:grid-cols-6 xl:grid-cols-8">{visibleItems.map((item) => <button key={item.id} type="button" onClick={() => createElement?.({ type: "text", text: item.value, content: item.value, width: 64, height: 64, fontFamily: item.fontFamily ?? "Arial", fontSize: item.fontSize ?? 42, fontWeight: "900", color: item.color ?? "#111111", meta: { fontSize: item.fontSize ?? 42, fontFamily: item.fontFamily ?? "Arial", color: item.color ?? "#111111", textAlign: "center", textShape: "straight", sticker: item.label, opacity: 1 } })} className="flex min-h-[52px] items-center justify-center rounded-[17px] border border-white/10 bg-white text-2xl shadow-sm transition hover:-translate-y-0.5 hover:border-violet-300 active:scale-95 [content-visibility:auto] [contain-intrinsic-size:52px]">{item.value}</button>)}</div>
