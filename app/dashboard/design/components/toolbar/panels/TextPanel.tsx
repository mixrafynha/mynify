...
const MOBILE_FEATURED_FONT_IDS = [
  "rubik-spray-paint",
  "special-elite",
  "black-ops-one",
  "bangers",
...
export default function TextPanel({
  ...
  const desktopCatalogFonts = useMemo(() => {
  ...
  const filteredFonts = useMemo(() => {
    if (isMobile) return allFilteredFonts.slice(0, MOBILE_PAGE_SIZE);
    return allFilteredFonts.slice(0, limit);
  ...
  useEffect(() => {
    ...
    const loadMobilePreviews = async () => {
      const loadedIds: string[] = [];

      for (const font of fontsToLoad) {
        if (cancelled) return;
        await loadEditorFont(font);
        loadedIds.push(font.id);
        await new Promise<void>((resolve) => {
          window.requestAnimationFrame(() => resolve());
        });
      }

      if (cancelled || !loadedIds.length) return;
  ...
  return (
    <div className="group relative overflow-hidden rounded-xl bg-white/[0.045] ring-1 ring-white/10 transition hover:bg-white/[0.065] active:scale-[0.98]">
      <button
        type="button"
        onClick={onAdd}
        className={`${compact ? "h-[54px]" : "h-[64px]"} flex w-full items-center justify-center px-2 py-2`}
        aria-label={`Add text with ${font.family}`}
        title={font.family}
      >
        <div
          className={`w-full truncate text-center font-black leading-none text-white transition-opacity duration-150 ${
      ...
      <button
        ...
        className={`absolute right-1.5 top-1.5 flex ${compact ? "h-6 w-6" : "h-7 w-7"} items-center justify-center rounded-xl backdrop-blur-md ${
          favorite
