  ...
  return (
    <div className="space-y-3 pb-6 text-white">
      <label className="flex h-11 items-center gap-2 rounded-2xl bg-white/[0.055] px-3 text-slate-400 ring-1 ring-white/10">
        <Search size={15} />
        <input value={query} onChange={(e) => { setQuery(e.target.value); setLimit(PAGE_SIZE); }} placeholder="Search 3D assets" className="min-w-0 flex-1 bg-transparent text-sm font-bold text-white outline-none placeholder:text-slate-600" />
      </label>
      ...
      <div className="grid grid-cols-3 gap-2 md:grid-cols-2 xl:grid-cols-3">
        {visible.map((item: SvgAssetPreset | any) => (
          <button key={item.id || `${item.category}-${item.label || item.title}`} type="button" onClick={() => addAsset(createElement, item)} className="group relative overflow-hidden rounded-2xl bg-white/[0.04] ring-1 ring-violet-400/15 active:scale-[0.98]">
