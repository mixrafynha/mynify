...
function addShape(createElement: ((element: any) => void) | undefined, shape: ShapePreset) {
  const src = createShapeAsset(shape);
  createElement?.({
    type: "image",
    src,
    imageUrl: src,
...
function IconsPanel({ createElement }: { createElement?: (element: any) => void }) {
  ...
  const categories = useMemo(
    () => Array.from(new Set([...SHAPE_CATEGORIES, ...shapes.map((item) => item.category)])),
    [shapes],
  );
  ...
  const items = useMemo(() => {
    ...
    return shapes.filter((item) => (category === "All" || item.category === category) && (!term || `${item.label} ${item.category}`.toLowerCase().includes(term)));
  }, [category, deferredQuery, shapes]);
  ...
  return (
    <div className="space-y-3 pb-5 text-white">
      <label className="flex h-10 items-center gap-2 rounded-xl border border-violet-300/20 bg-white/[0.075] px-3 text-violet-200 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)] transition focus-within:border-violet-300/45 focus-within:bg-white/[0.105]">
        ...
        <input value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="Search shapes" className="min-w-0 flex-1 bg-transparent text-sm font-semibold text-white outline-none placeholder:text-violet-100/55" />
      </label>

      <div className="flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {["All", ...categories].map((item) => {
          const active = category === item;
          return <button key={item} type="button" onClick={() => { setCategory(item); setPage(1); }} className={`shrink-0 rounded-full border px-3.5 py-2 text-[12px] font-extrabold leading-none transition active:scale-95 ${active ? "border-violet-200/70 bg-violet-500 text-white shadow-[0_8px_22px_rgba(139,92,246,0.32)]" : "border-violet-300/18 bg-white/[0.055] text-violet-100/80 hover:border-violet-300/35 hover:bg-white/[0.09] hover:text-white"}`}>{item}</button>;
        })}
      </div>
