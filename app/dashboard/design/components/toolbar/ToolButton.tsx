...
export default function ToolButton({ icon, label, active = false, disabled = false, badge, onClick }: ToolButtonProps) {
  return (
    <button
      ...
      onClick={onClick}
      className={clsx(
        "relative flex h-[41px] min-w-[44px] shrink-0 select-none flex-col items-center justify-center gap-0.5 rounded-lg px-1 text-[8px] font-bold leading-none touch-manipulation transition-colors duration-75 active:scale-[0.99]",
        active
    ...
    >
      ...
      <span className={clsx("flex items-center justify-center", active ? "text-white" : "text-violet-100/75")}>{icon}</span>
      <span className="max-w-[42px] truncate">{label}</span>
    </button>
  );
}
