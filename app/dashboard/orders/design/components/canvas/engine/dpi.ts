export type DPIQuality = "excellent" | "good" | "risk" | "poor" | "unknown";
...
export const TARGET_PRINT_DPI = 400;
...
export function getQualityStatusLabel(dpi: number | null | undefined): QualityStatus {
  const value = Number(dpi);
  if (!Number.isFinite(value) || value <= 0) return "Unknown";
  if (value >= TARGET_PRINT_DPI) return "Excellent";
  if (value >= 300) return "Good";
  if (value >= 200) return "Risk";
