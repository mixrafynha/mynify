...
import { ASSETS_3D, svgToDataUrl, type SvgAssetPreset } from "../data";
...
...imageDpiMeta(item, "image", { qualityMode: "3d-print-asset-400dpi" }),
      opacity: 1,
    },
  });
}
...
export default function Assets3DPanel({ createElement }: { createElement?: (element: any) => void }) {
  const categories = useMemo(() => ["All", ...Array.from(new Set((ASSETS_3D as any[]).map((item) => item.category)))], []);
  ...
  const items = useMemo(() => {
    ...
    return (ASSETS_3D as any[]).filter((item) => {
  ...
  return (
    <div className="space-y-3 pb-6 text-white">
      <label className="flex h-9 items-center gap-2 rounded-xl border border-slate-200 bg-white px-2.5 text-slate-500">
        <Search size={15} />
        <input value={query} onChange={(e) => { setQuery(e.target.value); setLimit(PAGE_SIZE); }} placeholder="Search 3D assets" className="min-w-0 flex-1 bg-transparent text-xs font-semibold text-slate-900 outline-none placeholder:text-slate-400" />
      </label>

      <div className="flex gap-1.5 overflow-x-auto pb-0.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {categories.map((item) => (
          <button key={item} type="button" onClick={() => { setCategory(item); setLimit(PAGE_SIZE); }} className={`shrink-0 rounded-full px-2.5 py-1.5 text-[11px] font-black transition-colors active:scale-95 ${category === item ? "bg-slate-950 text-white" : "bg-white text-slate-600 ring-1 ring-slate-200"}`}>
            {item}
          </button>
        ))}
      </div>
        ...
        {visible.map((item: SvgAssetPreset | any) => (
          <button key={item.id || `${item.category}-${item.label || item.title}`} type="button" onClick={() => addAsset(createElement, item)} className="group relative overflow-hidden rounded-xl bg-white ring-1 ring-slate-200 active:scale-[0.98]">
            ...
            <div className="absolute inset-x-0 bottom-0 bg-white/90 p-1.5 text-left">
              ...
              <p className="text-[9px] font-bold text-white/60">3D</p>
            </div>
          </button>
        ))}
      </div>
      ...
      {!items.length && <div className="py-10 text-center text-sm font-bold text-slate-500">No 3D assets found.</div>}
    </div>
  );
}
