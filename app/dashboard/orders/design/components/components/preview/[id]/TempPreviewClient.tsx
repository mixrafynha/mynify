      ...
      try {
        ...
        const response = await fetch("/api/products/generate-own-mockup", {
          ...
          body: JSON.stringify({
            productId: activeProductId,
            designImage,
            // IMPORTANT: production preview must use the captured PNG only.
            // Sending elements here can make the API render the design twice.
            elements: [],
            frontElements: [],
            backElements: [],
            printBox,
            ...
            mockupColor: previewData.mockupColor || "#ffffff",
            editorArea: { width: MOCKUP_AREA.width, height: MOCKUP_AREA.height },
            exportArea: { width: MOCKUP_AREA.width, height: MOCKUP_AREA.height },
            mockupArea: { width: MOCKUP_AREA.width, height: MOCKUP_AREA.height },
            ai: true,
            modelMockup: true,
            modelImages: Array.isArray(previewData.modelImages) ? previewData.modelImages : [],
          }),
        });
          ...
          >
            {captureElements.map((el: any, index: number) => {
              const style: React.CSSProperties = {
                ...
                transform: `rotate(${Number(el.meta?.rotation || 0)}deg) scale(${el.meta?.flipX ? -1 : 1}, ${el.meta?.flipY ? -1 : 1})`,
