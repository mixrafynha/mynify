...
import { captureProductionPreview, downloadDataUrl } from "./services/previewCapture";
...
function normalizeImages(value: any): string[] {
  ...
  return normalizeImages(value.mockupImages || value.images || value.imageUrls || value.urls || value.mockupUrl || value.imageUrl || value.url || value.data);
}

function uniqueImages(images: string[]) {
  return images.filter(Boolean).filter((img, index, arr) => arr.indexOf(img) === index);
}
...
function ProductionPreview({
  ...
  const [activeImageIndexBySide, setActiveImageIndexBySide] = useState<Partial<Record<PreviewSide, number>>>({});
  ...
  const activeImageIndex = Math.min(activeImageIndexBySide[activeSide] || 0, Math.max(0, sideImages.length - 1));
  ...
  const qualitySummary = useMemo(() => {
    ...
    const values = elements.map((el: any) => Number(el?.meta?.effectiveDpi ?? el?.meta?.dpi)).filter((value: number) => Number.isFinite(value) && value > 0);
    const minDpi = values.length ? Math.min(...values) : null;
    const status = !values.length ? "Unknown" : minDpi >= 300 ? "Excellent" : minDpi >= 200 ? "Good" : minDpi >= 150 ? "Risk" : "Poor Print Quality";
    const poor = elements.filter((el: any) => {
      const dpi = Number(el?.meta?.effectiveDpi ?? el?.meta?.dpi);
      return Number.isFinite(dpi) && dpi > 0 && dpi < 150;
    }).length;
    return { count: elements.length, minDpi, status, poor };
  }, [current?.elements]);
 ...
 const requestBody = useMemo(() => {
  ...
  return {
    productId: input.productId || preview.productId || "tshirt",
    category: input.category || preview.category || "tshirt",
    ...
    // CRÍTICO: escala real do editor
    editorArea: {
      width: exportResolution.width || 1024,
      height: exportResolution.height || 1024,
    },

    // CRÍTICO: output final esperado
    mockupArea: {
      width: exportResolution.width || 1024,
      height: exportResolution.height || 1024,
    },
    ...
    designImage: input.designImage || null,
  ...
  const generateMockup = useCallback(async (force = false) => {
    if (requestLockRef.current) return;
    if (!force && (generatedBySide[activeSide] || []).length > 1) return;
    ...
    try {
      ...
      console.info("MOCKUP REQUEST", {
  productId: requestBody.productId,
  category: requestBody.category,
  side: requestBody.side,
  hasDesignImage: Boolean(requestBody.designImage),
  elements: requestBody.elements?.length || 0,
  printBox: requestBody.printBox,
  safeArea: requestBody.safeArea,
  editorArea: requestBody.editorArea,
  mockupArea: requestBody.mockupArea,
});
      ...
      if (!response.ok || data?.success === false) {
        throw new Error(data?.error || data?.message || `AI mockup generation failed (${response.status})`);
      }
      ...
      const images = uniqueImages([
        ...
        ...normalizeImages(data?.localMockupImages),
        ...normalizeImages(data?.localGarmentImage),
      ]);

      if (!images.length) throw new Error("Mockup API did not return images.");
      ...
      setBackendError(error?.message || "AI mockup generation failed. Showing clean product preview.");
      setGeneratedBySide((prev) => ({ ...prev, [activeSide]: [] }));
    } finally {
      setGenerating(false);
      requestLockRef.current = false;
    }
  }, [activeSide, generatedBySide, requestBody]);

  useEffect(() => {
    if ((generatedBySide[activeSide] || []).length) return;
    generateMockup(false);
  }, [activeSide, generateMockup, generatedBySide]);

  async function downloadPreview() {
    const dataUrl = await captureProductionPreview(captureRef.current);
    if (!dataUrl) return;
    downloadDataUrl(dataUrl, `mynify-${preview.productId}-${activeSide}-preview.png`);
  }

  return (
    <div className="relative flex h-dvh w-screen flex-col overflow-hidden bg-[#f4f1ec] text-black" data-mynify-preview-root>
      ...
      <div className="absolute left-3 top-[calc(env(safe-area-inset-top)+66px)] z-40 max-w-[calc(100vw-24px)] rounded-[24px] border border-black/5 bg-white/88 px-4 py-3 text-black shadow-[0_18px_55px_rgba(0,0,0,.14)] backdrop-blur-2xl sm:left-4 sm:top-20">
        <p className="text-[10px] font-black uppercase tracking-[0.14em] text-black/45">Print readiness</p>
        <div className="mt-1 flex items-end gap-2">
          <span className="text-xl font-black tracking-[-0.04em]">{qualitySummary.status}</span>
          <span className="pb-0.5 text-xs font-bold text-black/45">{qualitySummary.minDpi ? `${qualitySummary.minDpi} DPI min` : "DPI unknown"}</span>
        </div>
        <p className="mt-1 text-[11px] font-bold text-black/50">{qualitySummary.count} elements · safe area + print area checked{qualitySummary.poor ? ` · ${qualitySummary.poor} poor` : ""}</p>
      </div>
          ...
          <div className="flex max-w-full gap-2 overflow-x-auto rounded-[28px] border border-black/5 bg-white/88 p-2 shadow-[0_18px_55px_rgba(0,0,0,.18)] backdrop-blur-2xl">
            {sideImages.map((img, index) => (
              <button
                key={`${img}-${index}`}
                type="button"
                onClick={() => setActiveImageIndexBySide((prev) => ({ ...prev, [activeSide]: index }))}
                className={`h-16 w-16 shrink-0 overflow-hidden rounded-2xl border bg-white transition active:scale-95 sm:h-20 sm:w-20 ${
                  index === activeImageIndex ? "border-black shadow-[0_0_0_2px_rgba(0,0,0,.18)]" : "border-black/10 opacity-70 hover:opacity-100"
                }`}
                aria-label={`Select pose ${index + 1}`}
              >
                <img src={img} alt={`Pose ${index + 1}`} className="h-full w-full object-cover" draggable={false} />
              </button>
            ))}
          </div>
        </div>
