...
function ProductionPreview({
  ...
  const requestBody = useMemo(
    () => ({
      ...
      poses: ["studio-front", "streetwear-front", "three-quarter", "lifestyle-close", "urban-walk", "close-crop"],
    }),
    [activeSide, current, input, preview.category, preview.mockupColor, preview.productId],
  );
  ...
  const generateMockup = useCallback(async (force = false) => {
    ...
    try {
      ...
      if (!response.ok || data?.success === false) {
        throw new Error(data?.error || data?.message || "AI mockup generation failed");
      }
